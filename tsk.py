...
_IMAGE_FILE_EXTENSIONS = ['jpg', 'png', 'svg']
...
class sky(object):
    ...
    def _create_sprite_type_from_module(sprite_type_module, image_filename):
        ...
        methods = {}
        for (module_attr_name, obj) in sprite_type_module.__dict__.items():
            # NOTE: hasattr can safely be called on most objects, but some
            #       objects that define __getattr__() raise an error that
            #       hasattr does not catch, like tsk.sky._Sprites.__getattr__().
            #       
            #       In Python 2 and DPython 2, hasattr() caught *all*
            #       exceptions. In Python 3 and DPython 3, hasattr() only
            ...
            methods[module_attr_name] = staticmethod(obj) \
                if has_name and obj.__name__.startswith('before_program_starts') \
                else obj
    ...
    class _Sprites(object):
        # TODO: Try to find a different way to implement this API that does not
        #       require raising NameError. Raising a NameError instead of
        #       AttributeError means that hasattr() generally cannot be used on
        #       an instance of _Sprites, or extra handling is required, such as
        #       is done in _create_sprite_type_from_module().
        #       
        #       In Python 2 and DPython 2, hasattr() caught *all* exceptions. In
        #       Python 3 and DPython 3, hasattr() only catches AttributeError.
        #       
        #       Looking at _resolve_sprite_instance_reference(), since it
        #       already explicitly raises a NameError, perhaps it's already safe
        #       to raise AttributeError here instead of NameError? Callers like
        #       _resolve_sprite_instance_reference() would need to be updated to
